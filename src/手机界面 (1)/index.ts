import { createApp, h, render, Teleport } from 'vue';
...
async function worldBookUpdate() {
  try {
    ...
    if (!iniVer || Number(bookVersion) > Number(iniVer)) {
      ...
      const aaa = sharedState.ST.getEntry(['手机-规则重述'], true);
      if (aaa) {
...
function CheckRegex() {
  ...
  const phoneRegex = regexes.find(item => item['find_regex'] === '/<phone>[\\s\\S]+?<\\/phone>/g');
